...
func (ic ipahealthcheckCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if sudo {
		chown := exec.Command("sudo", "chown", "root", tmpFile.Name())
		chown.Run()
		chmod := exec.Command("sudo", "chmod", "+r", tmpFile.Name())
		chmod.Stderr = os.Stderr
		err = chmod.Run()
		if err != nil {
			log.Infof("chmod returned errors: %v", err)
		} else {
			log.Info("used sudo to change ownership and permissions of: ", tmpFile.Name())
		}
	}
